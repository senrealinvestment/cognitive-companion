...
SERVICE_FAILURES = (
    [
        (
            sdk.TypeSafeAPIResponseValidationError(
                200,
                SENTINEL,
                httpx2.Headers({"Authorization": SENTINEL}),
                "answers.algorithm",
            ),
            "invalid_response",
        ),
    ...
    + [
        (
            sdk.TypeSafeAPIError(
                status, SENTINEL, httpx2.Headers({"Authorization": SENTINEL})
            ),
            reason,
        )
...
INVALID_RESPONSES = [
    ...
    response(answer(choice="unknown")),
    response(answer(probabilities={})),
    response(answer(probabilities=dict(PROBABILITIES, extra=0))),
    response(answer(probabilities={"anaphylaxis": 0.8, "other_or_unclear": 0.2})),
    response(answer(probabilities=dict(PROBABILITIES, anaphylaxis=0.5))),
    *[
